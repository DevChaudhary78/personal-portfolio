// Next.js API route support: https://nextjs.org/docs/api-routes/introduction
import type { NextApiRequest, NextApiResponse } from 'next'

type Quote = {
    quote: String
    speaker: String
    anime: String
}

export default function handler(
    req: NextApiRequest,
    res: NextApiResponse<Quote[]>
) {
    const quotes: Quote[] = [
        {
            quote: "Knowing what it feels to be in pain, is exactly why we try to be kind to others.",
            speaker: "Jiraya",
            anime: "Naruto"
        },
        {
            quote: "It's not about how hard you hit. It's about how hard you can get hit and keep moving forward.",
            speaker: "Rocky Balboa",
            anime: "Rocky"
        },
        {
            quote: "People, who can't throw something important away, can never hope to change anything.",
            speaker: "Armin Arlert",
            anime: "Attack on Titan"
        },
        {
            quote: "The only true fear is the fear of the unknown.",
            speaker: "Guts",
            anime: "Berserk"
        },
        {
            quote: "The only thing we're allowed to do is believe. We can't change anything.",
            speaker: "Makise Kurisu",
            anime: "Steins;Gate"
        },
        {
            quote: "If you can't find a reason to fight, then you shouldn't be fighting.",
            speaker: "Akame",
            anime: "Akame ga Kill!"
        },
        {
            quote: "Whatever you do, enjoy it to the fullest. That is the secret of life.",
            speaker: "Rider",
            anime: "Fate/Zero"
        },
        {
            quote: "If you don't like your destiny, don't accept it. Instead, have the courage to change it the way you want it to be.",
            speaker: "Naruto Uzumaki",
            anime: "Naruto"
        },
        {
            quote: "The world isn't perfect. But it's there for us, trying the best it can. That's what makes it so damn beautiful.",
            speaker: "Roy Mustang",
            anime: "Fullmetal Alchemist"
        },
        {
            quote: "The only true meaning is found within human relationships.",
            speaker: "Sora",
            anime: "No Game No Life"
        },
        {
            quote: "Believe in yourself. Not in the you who believes in me. Not the me who believes in you. Believe in the you who believes in yourself.",
            speaker: "Kamina",
            anime: "Tengen Toppa Gurren Lagann"
        },
        {
            quote: "A lesson without pain is meaningless. That's because no one can gain without sacrificing something. But by enduring that pain and overcoming it, he shall obtain a powerful, unmatched heart.",
            speaker: "Edward Elric",
            anime: "Fullmetal Alchemist"
        },
        {
            quote: "The world is merciless, but it's also very beautiful.",
            speaker: "Mikasa Ackerman",
            anime: "Attack on Titan"
        },
        {
            quote: "The only thing we're allowed to do is believe. We can't change anything.",
            speaker: "Okabe Rintarou",
            anime: "Steins;Gate"
        },
        {
            quote: "I want you to be happy. I want you to laugh a lot. I don’t know what exactly I’ll be able to do for you, but I’ll always be by your side.",
            speaker: "Kagome Higurashi",
            anime: "Inuyasha"
        },
        {
            quote: "We don’t know what kind of people we truly are until the moment before our deaths.",
            speaker: "Nakajima Youko",
            anime: "The Twelve Kingdoms"
        },
        {
            quote: "The world’s not perfect, but it’s there for us, trying the best it can. That’s what makes it so damn beautiful.",
            speaker: "Roy Mustang",
            anime: "Fullmetal Alchemist"
        },
        {
            quote: "A lesson without pain is meaningless. For you cannot gain something without sacrificing something else in return. But once you have recovered it and made it your own... You will gain an irreplaceable Fullmetal heart.",
            speaker: "Edward Elric",
            anime: "Fullmetal Alchemist"
        },
        {
            quote: "If you can't find a reason to fight, then you shouldn't be fighting.",
            speaker: "Akame",
            anime: "Akame ga Kill!"
        },
        {
            quote: "The only true fear is the fear of the unknown.",
            speaker: "Guts",
            anime: "Berserk"
        },
        {
            quote: "Don’t forget. Always, somewhere, someone is fighting for you. As long as you remember her, you are not alone.",
            speaker: "Madoka Kaname",
            anime: "Puella Magi Madoka Magica"
        },
        {
            quote: "If you don’t take risks, you can’t create a future.",
            speaker: "Monkey D. Luffy",
            anime: "One Piece"
        },
        {
            quote: "The only way to truly escape the mundane is for you to constantly be evolving.",
            speaker: "Sora",
            anime: "No Game No Life"
        },
        {
            quote: "The only true wisdom is in knowing you know nothing.",
            speaker: "Socrates",
            anime: "Soul Eater"
        },
        {
            quote: "The world is a cruel place, but it is also very beautiful.",
            speaker: "Mikasa Ackerman",
            anime: "Attack on Titan"
        },
        {
            quote: "You don't become the Hokage to be acknowledged by everyone. The one who is acknowledged by everyone becomes the Hokage.",
            speaker: "Naruto Uzumaki",
            anime: "Naruto"
        },
        {
            quote: "There are no miracles in this world. There are only people with strong wills.",
            speaker: "Trafalgar Law",
            anime: "One Piece"
        },
        {
            quote: "I'll take a potato chip... and eat it!",
            speaker: "Light Yagami",
            anime: "Death Note"
        },
        {
            quote: "Believe in yourself. Not in the you who believes in me. Not the me who believes in you. Believe in the you who believes in yourself.",
            speaker: "Kamina",
            anime: "Tengen Toppa Gurren Lagann"
        },
        {
            quote: "You can't always hold on to the things that are important. By letting them go, we gain something else.",
            speaker: "Edward Elric",
            anime: "Fullmetal Alchemist"
        },
        {
            quote: "It's not about whether you get knocked down, it's whether you get up.",
            speaker: "Saitama",
            anime: "One-Punch Man"
        },
        {
            quote: "No matter what happens, never give up on your dreams.",
            speaker: "Frog",
            anime: "Chrono Trigger: The Movie"
        },
        {
            quote: "Hard work betrays none, but dreams betray many.",
            speaker: "Hachiman Hikigaya",
            anime: "My Teen Romantic Comedy SNAFU"
        },
        {
            quote: "We are not defined by our pasts, but by the choices we make in the present.",
            speaker: "Alucard",
            anime: "Hellsing Ultimate"
        },
        {
            quote: "It's not a matter of can or can't. There are some things in life you just do.",
            speaker: "Lightning",
            anime: "Final Fantasy XIII"
        },
    ]

    res.status(200).json(quotes)
}
